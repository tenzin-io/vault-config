...
resource "vault_jwt_auth_backend_role" "github_repos" {
  ...
  token_policies = flatten([vault_policy.default_policy.name, [for p in each.value.secrets : format("github-repo-to-secrets-policy-%s", md5(p))]])
  bound_claims = {
    repository = each.key
  }
...
resource "vault_policy" "github_repo_to_secrets" {
  ...
  name     = "github-repo-to-secrets-policy-${md5(each.value)}"
  policy   = <<-EOT
    path "${each.value}" {
      capabilities = ["read"]
    }
